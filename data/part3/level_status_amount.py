...
if __name__ == "__main__":
        ...
        tuples = tuples.filter(lambda x : len(x) > 11).filter(lambda x : x[11] != '' and x[10]!= '')
        pairs = tuples.map(lambda x : ((x[11].strip(), x[10].strip()), 1))
        ...
        result.saveAsTextFile("level_status_amount.out")
        sc.stop()
